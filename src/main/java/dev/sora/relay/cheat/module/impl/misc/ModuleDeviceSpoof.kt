...
class ModuleDeviceSpoof : CheatModule("DeviceSpoof", CheatCategory.MISC) {
	...
	private val handlePacketOutbound = handle<EventPacketOutbound> {
		if (packet is LoginPacket) {
			...
			packet.extra = "." + Base64.getEncoder().withoutPadding().encodeToString(AbstractConfigManager.DEFAULT_GSON.toJson(body).toByteArray(Charsets.UTF_8)) + "."		} else if (platformValue && packet is PlayerAuthInputPacket) {
